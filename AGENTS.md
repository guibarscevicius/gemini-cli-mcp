...
8. Corrupt-history surfaces as MCP error (#119) — call `ask-gemini` to seed a session, corrupt its `turns` row via direct SQLite write against the non-`:memory:` DB, call `gemini-reply` against that session, expect an MCP error mentioning "corrupt turn data" (not a silently-fresh conversation).
