...
This repo's CHANGELOG is generated by release-please, which only sees Conventional Commits prefixes. **Any commit that removes or renames a public MCP surface (tool name, resource URI, env var, exported symbol) MUST use `feat!:` or include a `BREAKING CHANGE:` footer.**

`refactor:` is reserved for internal restructuring that does not affect user-visible behavior. Using it for a tool removal will silently omit the removal from the next CHANGELOG (this happened in PR #114 → issue #116).
...
8. Corrupt-history surfaces as MCP error (#119) — call `ask-gemini` to seed a session (note the `sessionId`), then set its `turns` column to a non-JSON string against the non-`:memory:` DB: `sqlite3 ~/.gemini-cli-mcp/sessions.db "UPDATE sessions SET turns = 'bad' WHERE id = '<sessionId>'"`. Call `gemini-reply` against that session and expect an MCP error mentioning "corrupt turn data" (not a silently-fresh conversation).
9. Notifications/cancelled kills running subprocess (#120) — start `ask-gemini` async, confirm a subprocess via `pgrep gemini`, send `notifications/cancelled` with the request id, expect `pgrep` count to drop and `gemini-poll` to report `status: cancelled`.
10. Non-JSON stdout surfaces in default config (#121) — start the server **without** `GEMINI_STRUCTURED_LOGS=1`, trigger an `ask-gemini` call that yields any non-JSON CLI line, confirm stderr contains `non-JSON stdout line dropped:`.
11. `historyPersisted: false` on appendTurn failure (#122) — force `appendTurn` to throw (e.g. corrupt the session row mid-flight), call `ask-gemini` wait-mode, expect the response to include `historyPersisted: false` and an `historyError` string; same check via `gemini-poll` for async callers.
